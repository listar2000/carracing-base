...
class FrictionDetector(contactListener):
    def __init__(self, env, grass_penalty=False):
    ...
    def _contact(self, contact, begin):
        ...
        # we need to add the reward back if giving penalty to grass
...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self, verbose=1, diff_road_col=True, grass_penalty=False):
        EzPickle.__init__(self)
        self.seed()

        self.diff_road_col: bool = diff_road_col
        self.grass_penalty: bool = grass_penalty

        self.contactListener_keepref = FrictionDetector(self, grass_penalty=self.grass_penalty)
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            # modified by star, when diff_road_col is turned off, visited & unvisited 
            # tiles will have the same color
            if self.diff_road_col:
                c = 0.01 * (i % 3)
    ...
    def step(self, action):
        ...
        if action is not None:  # First step without action, called from reset()
            if self.grass_penalty:
                # penalty for on grass is 1 per step
                self.reward -= 1.1
            else:
                self.reward -= 0.1
